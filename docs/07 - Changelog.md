...
- Contacts raccordé : nouvel interprète local déterministe (`contact-assistant-interpreter.ts`), sans appel IA payant, qui reconnaît une question de téléphone/e-mail/coordonnées « de <nom> » et exclut explicitement les questions d'OF (« numéro de l'OF-63596 ») pour ne pas les intercepter par erreur. La recherche par nom réutilise `filterContacts`/`sortContactsByName` du module Contacts, sans dupliquer la logique de recherche. En cas d'homonymes, l'assistant liste les correspondances (nom + société) et demande de préciser plutôt que de répondre au hasard — même principe de non-invention que le reste de l'assistant. Lecture directe et synchrone de `data.contacts`, sans passer par l'orchestrateur central : même architecture que la revue Actions déjà en place, réservée aux appels réseau/mutations à tracer.
- Planning par machine raccordé : `planning-assistant-interpreter.ts` gagne `resolveMachineQuery` (résout un nom ou identifiant de machine cité en texte libre vers une machine réelle des Réglages Production, par identifiant exact ou sous-chaîne de son nom d'affichage — même convention de recherche que le sélecteur de machine de l'Atelier ; jamais de choix silencieux entre deux machines proches, les candidats sont listés pour préciser) et `buildMachineOfListReply` (nombre d'OF demandé extrait du texte, par défaut 5, plafonné à 50, avec mention explicite du surplus non affiché). `planning-assistant-orchestration.ts` gagne `fetchOperationsForMachine`, passée par l'orchestrateur central comme la recherche par OF, même route `GET /api/erp/planning?machine=` que l'Atelier. Le tri par priorité réutilise `sortOperations` (`workshop-view-service.ts`, colonne priorité de l'Atelier) au lieu de redéfinir un ordre de priorité concurrent.
- `AssistantPanel.tsx` : les deux capacités répondent directement (lecture seule, aucune modification donc aucune confirmation à demander), câblées avant le repli sur la recherche par OF existante pour qu'une question par machine ne soit jamais absorbée à tort par le flux « quel OF ? ».
- 16 nouveaux tests unitaires purs (10 dans `tests/planning-assistant.test.mjs` pour la résolution/formatage machine, 8 dans le nouveau `tests/contact-assistant.test.mjs`). `npx tsc --noEmit`, `npm run lint`, `npm test` (474/474), `npm run build` tous verts.
- [ ] Recette manuelle dans le navigateur nécessaire pour l'utilisateur : demander le téléphone/e-mail d'un contact existant (cas unique, homonymes, contact introuvable), puis les OF les plus prioritaires d'une machine réelle (cas trouvé, machine ambiguë, machine introuvable).
