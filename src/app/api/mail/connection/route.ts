...
export async function GET() {
  try {
    ...
    return apiJson({ account: { ...account, status: status.state, connectedAt: status.connectedAt, lastSuccessfulSyncAt: status.lastSuccessfulSyncAt ?? account.lastSuccessfulSyncAt, error: status.error ?? null } });
  } catch (error) {
