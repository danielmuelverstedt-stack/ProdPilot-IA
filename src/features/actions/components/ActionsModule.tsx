...
import { Button, FilterBar, ModuleHeader, SearchInput, Select as UiSelect, Tabs } from "@/components/ui/ModuleUi";
...
export function ActionsModule() {
  ...
  return <div className="mx-auto max-w-7xl">
    <ModuleHeader
      ...
      actions={<Button onClick={() => setCreating(true)}>{view === "backlog" ? "+ Nouvelle idée" : "+ Nouvelle action"}</Button>}
    />
    {creating ? <ActionFormDialog origine="Manuel" mode={view === "backlog" ? "backlog" : "full"} onClose={() => setCreating(false)} /> : null}

    <div className="mt-6"><Tabs label="Vues des actions" value={view} onChange={(next) => setView(next as ActionsView)} items={[{ value: "current", label: "Liste des actions" }, { value: "backlog", label: <>À planifier{backlogCount > 0 ? <span className="ml-2 rounded-full bg-slate-200 px-2 py-0.5 text-xs text-slate-700">{backlogCount}</span> : null}</> }, { value: "team-planning", label: "Planification équipe" }]} /></div>
    ...
    {view === "team-planning" ? <TeamPlanningTab actions={topLevelActions} people={data.people} /> : <>
      <FilterBar className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <SearchInput value={search} onChange={(event) => setSearch(event.target.value)} onClear={() => setSearch("")} placeholder="Rechercher…" />
        {view === "current" ? <>
...
function Select({ value, onChange, values }: { value: string; onChange: (value: string) => void; values: Iterable<string> }) {
  return <UiSelect value={value} onChange={(event) => onChange(event.target.value)}>{[...values].map((item) => <option key={item}>{item}</option>)}</UiSelect>;
}
