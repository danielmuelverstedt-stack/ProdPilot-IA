...
function parseConfiguration(value: unknown): MailAiConfiguration | null {
  ...
  if (![value.maximumInputContextTokens, value.maximumAnalysisOutputTokens, value.maximumReplyOutputTokens, value.maximumRewriteOutputTokens, value.dailyHardLimit, value.longThreadWarningThreshold].every((item) => Number.isInteger(item) && Number(item) > 0 && Number(item) <= 100_000)) return null;
