...
import type { ErpOperationStatus, ErpPlanningOverview, ErpPlanningQueryResult, ErpPlanningRow } from "@/features/erp-import/types/erp-import";
...
export function ErpPlanningWorkspace() {
  ...
  const [search, setSearch] = useState(activePlanningView.filters.search);
  ...
  const [selectedOf, setSelectedOf] = useState<{ id: string; rows: ErpPlanningRow[] } | null>(null);
  ...
  const loadRows = useCallback(async () => {
    const params = new URLSearchParams({ page: String(page), pageSize: "100", sort: activePlanningView.sort });
    if (search.trim()) params.set("search", search.trim());
    if (view === "unmapped") params.set("machine", "unmapped");
    else if (activePlanningView.filters.machine) params.set("machine", activePlanningView.filters.machine);
    if (activePlanningView.filters.status) params.set("status", activePlanningView.filters.status);
    if (activePlanningView.filters.late) params.set("late", activePlanningView.filters.late);
    if (activePlanningView.filters.articleMultiplicity !== "all") params.set("articleMultiplicity", activePlanningView.filters.articleMultiplicity);
    if (view === "unmapped") params.set("validMachines", activeMachineIds.join(","));
    const response = await fetch(`/api/erp/planning?${params}`, { cache: "no-store" });
    const payload: unknown = await response.json();
    if (!response.ok) throw new Error(apiMessage(payload));
    setRows(payload as ErpPlanningQueryResult);
  }, [activeMachineIds, activePlanningView.filters.articleMultiplicity, activePlanningView.filters.late, activePlanningView.filters.machine, activePlanningView.filters.status, activePlanningView.sort, page, search, view]);
  ...
  useEffect(() => { const timer = window.setTimeout(() => { setSearch(activePlanningView.filters.search); setPage(1); }, 300); return () => window.clearTimeout(timer); }, [activePlanningView.filters.search]);
  ...
  async function importFiles(files: File[]) {
    ...
    } catch (nextError) { setError(nextError instanceof Error ? nextError.message : "L’import a échoué."); }
