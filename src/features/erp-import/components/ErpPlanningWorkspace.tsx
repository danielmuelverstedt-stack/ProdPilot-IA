...
export function ErpPlanningWorkspace() {
  ...
  const preservedMachineDifferences = useMemo(() => allOperations.filter((row) => row.hasMachineDifference), [allOperations]);
  ...
  return <div className="space-y-5">
    ...
    {view === "imports" ? <ImportsView overview={overview} busy={busy} onImport={importFiles} onMap={saveMapping} onDeleteMapping={deleteMapping} machines={machines} preservedMachineDifferences={preservedMachineDifferences} /> : null}
  </div>;
}
...
function ImportsView({ overview, busy, onImport, onMap, onDeleteMapping, machines, preservedMachineDifferences }: { overview: ErpPlanningOverview | null; busy: boolean; onImport: (files: File[]) => Promise<void>; onMap: (code: string, machineId: string) => Promise<void>; onDeleteMapping: (code: string) => Promise<void>; machines: ReturnType<typeof useSettings>["settings"]["production"]["machines"]; preservedMachineDifferences: OperationView[] }) {
