...
export function MachineDetail({ id }: { id: string }) {
  ...
  const fieldPoints = data.meetings.flatMap((meeting) => (meeting.fieldPoints ?? []).filter((point) => point.machineIds.includes(id)).map((point) => ({ ...point, meetingId: meeting.id })));
  ...
  const actions = data.actions.filter((item) => item.contextLinks.some((link) => (link.module === "machine" || link.module === "maintenance") && link.id === id));
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-4 rounded-2xl border border-[var(--app-border)] bg-white p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">{tab}</h2>{details ? <StatusPill tone={details.status === "En panne" ? "danger" : details.status === "Disponible" ? "success" : "warning"}>{details.status}</StatusPill> : null}</div>
      {tab === "Vue générale" ? <div className="mt-5"><MachineIdentityPanel machine={machine} departments={settings.production.departments} halls={settings.production.halls} openActionsCount={openActions.length} mappedErpCodes={mappedErpCodes} onSave={(patch) => updateSettings((draft) => { machineSettingsService.updateIdentity(draft, id, patch); }, "Machine modifiée")} /></div> : null}
      ...
      {tab === "Maintenance" ? <div className="mt-5"><h3 className="mb-3 font-semibold">Maintenance / Problèmes</h3><MaintenanceProblemsWorkspace machineId={id} />{events.length ? <div className="mt-6 border-t pt-5"><h3 className="font-semibold">Interventions planifiées</h3><div className="mt-3 grid gap-3">{events.map((event) => <article className="rounded-xl bg-slate-50 p-4" key={event.id}><div className="flex justify-between gap-2"><strong>{event.type}</strong><StatusPill tone={event.status === "Terminée" ? "success" : event.status === "En cours" ? "danger" : "warning"}>{event.status}</StatusPill></div><p className="mt-2 text-sm">{formatEuropeanDate(event.date, true)} · {event.durationHours} h · {event.responsible}</p><p className="mt-1 text-xs text-slate-500">{event.comment}</p></article>)}</div></div> : null}<MachineSavContactsPanel machineId={id} /><MachineConsumablesPanel machineId={id} /></div> : null}
      {tab === "Historique" ? <div className="mt-5 space-y-3">{[...events].reverse().map((event) => <p key={event.id} className="border-l-2 pl-3 text-sm">{formatEuropeanDate(event.date)} · {event.type} · {event.status}</p>)}{fieldPoints.length ? <div className="mt-4 border-t border-slate-100 pt-4"><h3 className="text-sm font-semibold">Remontées terrain</h3><div className="mt-2 space-y-2">{fieldPoints.map((point) => <p key={point.id} className="rounded-lg bg-slate-50 p-3 text-sm">{formatEuropeanDate(point.createdAt, true)} · {point.text} <span className="text-xs text-slate-500">· {point.meetingId}</span></p>)}</div></div> : null}{actions.length ? <div className="mt-4 border-t border-slate-100 pt-4"><h3 className="text-sm font-semibold">Actions liées</h3><div className="mt-2 space-y-2">{actions.map((action) => <Link key={action.id} href={`/actions/${action.id}`} className="block rounded-lg bg-slate-50 p-3 text-sm hover:bg-slate-100">{action.description} <span className="text-xs text-slate-500">· {action.statut}</span></Link>)}</div></div> : null}</div> : null}
