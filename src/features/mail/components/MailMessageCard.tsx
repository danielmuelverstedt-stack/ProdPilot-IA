...
export function MailMessageCard({ account, message, isOpen, replyRequested, onOpen, onPrepareReply, onCreateAction, onIgnore, onNotice }: {
  account: MailAccount;
  message: MailMessage;
  isOpen: boolean;
  ...
  return <article className={`rounded-3xl border border-[#dce5e0] bg-white shadow-[0_12px_35px_rgba(29,64,50,0.055)] ${isCompact ? "p-4" : "p-5 sm:p-6"} ${hasRightPane ? "xl:grid xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.8fr)] xl:gap-6" : ""}`}>
    <div className="flex flex-col gap-5 xl:grid xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
      <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${category.className}`}>{category.label}</span><span className="text-xs font-medium text-[#7a8982]">{mailProviderLabels[account.provider]} · {account.displayName}</span>{!message.isRead ? <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#1d694b]"><span aria-hidden="true" className="size-2 rounded-full bg-[#278a63]" />Non lu</span> : null}{message.isFlagged ? <span className="text-xs font-semibold text-amber-700">Avec indicateur</span> : null}</div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap xl:max-w-[390px] xl:justify-end"><ActionButton label={isOpen ? "Fermer" : "Ouvrir"} onClick={onOpen} /><ActionButton label="Proposer une réponse" onClick={onPrepareReply} primary /><ActionButton label="Créer une action" onClick={onCreateAction} /><ActionButton label="Ignorer" onClick={onIgnore} muted /></div>
    </div>
...
function ActionButton({ label, onClick, primary = false, muted = false }: { label: string; onClick: () => void; primary?: boolean; muted?: boolean }) { const color = primary ? "border-[#195c45] bg-[#195c45] text-white hover:bg-[#104432]" : muted ? "border-transparent bg-transparent text-[#7b8982] hover:bg-[#f3f5f4]" : "border-[#cbd7d1] bg-white text-[#40554b] hover:bg-[#f5f8f6]"; return <button type="button" onClick={onClick} className={`min-h-10 rounded-xl border px-3 text-xs font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#195c45] sm:text-sm ${color}`}>{label}</button>; }
