...
export function MailWorkspaceLoader() {
  ...
  if (state.kind === "connection-required") return <WorkspaceState title="Le compte actif n’est pas connecté" description={`${state.account.error ?? "Activez ou reconnectez un compte dans Réglages."} Compte actuel : ${state.account.displayName}.`} actions={<a href="/reglages/connexions/messagerie" className="rounded-xl bg-[var(--app-primary)] px-4 py-3 text-sm font-semibold text-white">Ouvrir les réglages</a>} />;
