...
export function PlanningGrid({ machines, days, blocks, canCreate, canEdit, canPrint, draggedId, dragOver, onDragStart, onDragEnd, onDragOver, onDrop, onAdd, onMove, onReorder, onPrint }: {
    ...
    <div className={styles.gridScroller}>
      <table className={styles.grid}>
        <thead>
          ...
          <tr>{days.map((day) => <th key={day.date} className={`${styles.dayHeader} ${day.isFriday ? styles.friday : ""}`}>{day.dayLabel}<span className="block font-normal text-slate-400">{day.dateLabel}</span></th>)}</tr>
        </thead>
        <tbody>{machines.map((machine) => <MachinePlanningRow key={machine.id} machine={machine} days={days} blocks={blocks.filter((block) => block.machineId === machine.id)} canCreate={canCreate} canEdit={canEdit} canPrint={canPrint} draggedId={draggedId} dragOver={dragOver} onDragStart={onDragStart} onDragEnd={onDragEnd} onDragOver={onDragOver} onDrop={onDrop} onAdd={onAdd} onMove={onMove} onReorder={onReorder} onPrint={onPrint} />)}</tbody>
      </table>
    </div>
  </div>;
}
