...
export function AiUsageDashboard({ summary, query, pending, error, onQueryChange }: { summary: AiUsageSummary | null; query: AiUsageQuery; pending: boolean; error: string | null; onQueryChange: (query: AiUsageQuery) => void }) {
  return <SettingsPanel title="Utilisation et budget" description="Métriques internes sans corps d’e-mail, prompt complet, destinataire, secret ni jeton OAuth.">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <Field label="Opération"><select className={inputClass} value={query.operation ?? "all"} onChange={(event) => onQueryChange({ ...query, operation: event.target.value as AiOperationType | "all" })}><option value="all">Toutes</option><option value="mail_analysis">Analyses</option><option value="mail_reply">Réponses</option><option value="mail_rewrite">Réécritures</option><option value="mail_conversation">Conversations</option><option value="connection_test">Tests de connexion</option></select></Field>
...
function Dashboard({ summary }: { summary: AiUsageSummary }) {
  ...
  return <div className="mt-5 space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Metric label="Analyses" value={summary.operations.mail_analysis} /><Metric label="Réponses" value={summary.operations.mail_reply} /><Metric label="Réécritures" value={summary.operations.mail_rewrite} /><Metric label="Conversations" value={summary.operations.mail_conversation} /><Metric label="Requêtes bloquées" value={summary.blockedRequests} />
