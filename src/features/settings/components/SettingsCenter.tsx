...
const categories = ["Général", "Personnalisation", "Connexions", "Mails", "ERP", "Production", "Réunions", "IA", "Notifications", "Utilisateurs", "Sauvegardes", "Journal"] as const;
...
export function SettingsCenter() {
  ...
  return <div className="mx-auto max-w-[1500px]"><header className="mb-6"><p className="text-xs font-bold uppercase tracking-widest text-[var(--app-primary)]">Administration locale</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[var(--app-text)]">Centre de réglages</h1><p className="mt-2 max-w-3xl text-slate-600">Personnalisez l’interface, préparez les référentiels et gérez les connexions disponibles sans modifier l’ERP.</p></header><div className="grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]"><nav aria-label="Catégories de réglages" className="h-fit rounded-2xl border border-[var(--app-border)] bg-white p-2 shadow-[var(--app-shadow-sm)]"><ul>{categories.map((item) => <li key={item}><button className={`w-full rounded-lg px-3 py-2.5 text-left text-sm font-medium transition ${category === item ? "bg-[var(--app-primary)] text-white shadow-[var(--app-shadow-sm)]" : "text-slate-600 hover:bg-slate-50"}`} onClick={() => setCategory(item)}>{item}</button></li>)}</ul></nav><div className="min-w-0">{category === "Personnalisation" && <div className="mb-4 flex gap-2 overflow-x-auto pb-1">{personalization.map((item) => <button key={item} onClick={() => setPersonalizationTab(item)} className={`shrink-0 rounded-full border px-3 py-2 text-xs font-semibold transition ${personalizationTab === item ? "border-[color-mix(in_srgb,var(--app-primary)_45%,transparent)] bg-[color-mix(in_srgb,var(--app-primary)_10%,white)] text-[var(--app-primary)]" : "border-[var(--app-border)] bg-white text-slate-600 hover:bg-slate-50"}`}>{item}</button>)}</div>}<SettingsContent category={category} personalizationTab={personalizationTab} /></div></div></div>;
}

function SettingsContent({ category, personalizationTab }: { category: Category; personalizationTab: (typeof personalization)[number] }) {
  if (category === "Mails") return <MailMemorySettingsPanel />;
  ...
  if (category === "Connexions") return <SettingsPanel title="Connexions" description="Les intégrations externes restent isolées des données de démonstration."><div className="grid gap-3 sm:grid-cols-2"><Link className={buttonClass} href="/reglages/connexions/messagerie">Messagerie Google, Microsoft et démonstration</Link><div className="rounded-xl border border-dashed p-4 text-sm text-slate-500"><strong className="block text-slate-700">ERP / Supabase</strong>Non connecté dans cette version.</div></div></SettingsPanel>;
  if (category === "ERP") return <SettingsPanel title="ERP" description="Imports Excel locaux et contrôlés, sans écriture directe dans l’ERP."><div className="space-y-3 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-900"><p><strong>Moteur d’import actif.</strong> Les exports Top et Details sont validés, historisés et projetés dans le Planning.</p><div className="flex flex-wrap gap-2"><Link className={buttonClass} href="/planning">Imports et correspondances machines</Link><Link className={buttonClass} href="/qualite-erp">Qualité des données ERP</Link></div></div></SettingsPanel>;
  ...
  if (category === "IA") return <AiSettingsPanel />;
