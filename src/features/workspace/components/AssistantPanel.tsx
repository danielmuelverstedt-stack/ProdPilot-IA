...
export function AssistantPanel() {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice("L’assistant est en préparation. Votre demande n’a pas été envoyée.");
  }
  ...
  return (
    <section aria-labelledby="assistant-title" className="mt-8 overflow-hidden rounded-3xl bg-[#123d30] p-6 text-white shadow-[0_20px_55px_rgba(18,61,48,0.18)] sm:p-8">
      <div className="max-w-2xl">
        ...
        <p className="mt-2 text-sm leading-6 text-emerald-50/70">Posez une question sur vos priorités. Cette première version ne transmet encore aucune donnée.</p>
      </div>
      <form onSubmit={handleSubmit} className="mt-6">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p aria-live="polite" className="text-sm text-emerald-100/75">{notice ?? "Aucune donnée ne quitte l’application."}</p>
