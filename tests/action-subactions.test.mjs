...
function action(overrides) {
  return { id: "ACT-900", dateEncodage: "2026-07-01", introduitPar: "Daniel", origine: "Manuel", contextLink: null, description: "Test", responsable: "", echeance: "2026-07-01", statut: "À faire", dateCloture: null, remarque: null, priority: null, responsableId: null, estimatedHours: null, plannedWeek: null, planningOrder: null, parentActionId: null, ...overrides };
}
...
test("la revue des actions en réunion exclut aussi les sous-actions, même règle que le module Actions", async () => {
  const source = await read("src/features/meetings/components/MeetingActionReview.tsx");
  assert.match(source, /import \{ isSubAction \} from "@\/features\/actions\/services\/action-status"/);
  assert.match(source, /item\.statut === "À faire" && !isSubAction\(item\)/);
});
...
test("la fiche action permet d'ouvrir/créer des sous-actions, réutilisant le même tableau (ActionGroupedList) que le reste du module", async () => {
  ...
  assert.match(detail, /<button className={secondaryButton} onClick={\(\) => setCreatingSubAction\(true\)}>\+ Sous-action<\/button>/);
