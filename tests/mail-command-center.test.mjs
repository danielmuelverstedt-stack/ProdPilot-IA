...
test("le style reste sombre, aéré et réduit ses animations", async () => { const standby = await read("src/features/mail-assistant/components/MailCommandCenterStandby.tsx"); const shell = await read("src/features/mail-assistant/components/MailSessionShell.tsx"); assert.match(standby, /#0b0e0d/); assert.match(standby, /sm:text-7xl/); assert.match(standby, /motion-reduce/); assert.match(shell, /bg-\[#0b0e0d\]/); });
